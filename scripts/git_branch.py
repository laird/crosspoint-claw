...
def inject_version(env):
    ...
    version_string = f'{base_version} ({sha})'

    # Write build_version.generated.h so the -include picks up the current version.
    # This ensures the displayed version is always up-to-date regardless of SCons
    # define ordering (the header #define takes precedence over -D on the command line).
    ...
    try:
        existing = open(header_path).read() if os.path.exists(header_path) else ''
        if existing != header_content:
            with open(header_path, 'w') as f:
                f.write(header_content)
            print(f'[git_branch] Updated build_version.generated.h: {version_string}')
    except OSError as e:
        warn(f'Could not write build_version.generated.h: {e}')
